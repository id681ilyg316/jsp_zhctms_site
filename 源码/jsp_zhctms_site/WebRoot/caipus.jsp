					...
					<div class="row">
						<div class="col-md-12">
							...
							<%
							    ...
							    %>
								<li class="lm-imgItem">
							        <a href="caipu.jsp?id=<%=goodsmap.get("id") %>"><img src="upfile/<%=goodsmap.get("filename") %>"/></a>
					                <div class="buttons">
											<a class="btn cart" href="javascript:tocar(<%=goodsmap.get("id") %>)" title="加入购物车"><span class="glyphicon glyphicon-shopping-cart"></span></a>
											<a class="btn wishlist" href="/zhctms_site/zhctms_site?ac=addfav&goodid=<%=goodsmap.get("id") %>&page=index.jsp" title="收藏此菜谱"><span class="glyphicon glyphicon-heart"></span></a>
								    </div>
								    <div class="caption">
											<div class="name lm-name"><a href="caipu.jsp?id=<%=goodsmap.get("id") %>"><%=goodsmap.get("goodname").toString().length()>13?goodsmap.get("goodname").toString().substring(0,13)+"...":goodsmap.get("goodname") %></a></div>
											<div class="price">
...
function tocar(gid){     
	 $.ajax({  
	        type: "POST",      
	         url: "/zhctms_site/zhctms_site?ac=tocar", //servlet的名字     
	          data: "gid="+gid+"&sl=1", 
	         success: function(data){       
	    if(data=="true"){     
	    	$("#cartsinfo").show();
	     	$("#cartsinfo").html("该菜谱已加入到购物车");
	    }else{    
	    	$("#cartsinfo").show(); 
